...
export default {
  ...
  mounted () {
    if (this.$cookies.get('sdt')) {
      this.$axios.$post('/getsettings/logging', { jwt: this.$cookies.get('sdt'), id: this.id }).then((res) => {
        this.loggedIn = true
  ...
  methods: {
    submit () {
      this.$axios.$post('/setsettings/logging', { jwt: this.$cookies.get('sdt'), id: this.id, settings: this.settings }).then((res) => {
        if (res.success) {
